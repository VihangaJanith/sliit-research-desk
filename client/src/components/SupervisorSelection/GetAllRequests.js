...
useEffect(() => {
 ...
 const res = await fetch('http://localhost:8000/supervisorselection/');
...
const handleDelete = async(id) => {
  try{
    const res = await fetch(`http://localhost:8000/supervisorselection/${id}`, {
    method : 'DELETE'
  })
  if(res.ok){
...
<h1>All Reqested Supervisors </h1>
         ...
         {crs?.map((cr , index) => (
           ...
           <div className="col-md-8 card me-3 mt-1 mb-3 p-0" key={cr.id}>
                   ...
                   <p> StudentID : {cr.studentid}</p>
                        ...
                        <a  className="col-md-9 card me-3 mt-2 p-1" style={{textDecoration: 'none', color:'crimson'}}>Reqested Supervisor ID  : {cr.superid}</a> 
                        <a  className="col-md-9 card me-3 mt-2 p-1" style={{textDecoration: 'none', color:'crimson'}}>Status from Supervisor : {cr.approval}</a> 
