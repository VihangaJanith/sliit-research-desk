...
import axios from "axios";
 ...
 function RequestStatusCo({match}) {
    ...
    useEffect(() => {
       fetch(`http://localhost:8000/cosupervisorselection/${match.params.id}`)
       .then((res) => res.json())
    ...
    const handleSubmit= async e =>{
        e.preventDefault();
        await axios.put(`http://localhost:8000/cosupervisorselection/${match.params.id}`,prds);
...
export default RequestStatusCo